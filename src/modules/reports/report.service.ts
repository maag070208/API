...
// Operational roles are those that perform scans, rounds, and reports
const OPERATIONAL_ROLES: Role[] = [Role.GUARD, Role.SHIFT_GUARD, Role.MANTENIMIENTO];
...
export const getTopPerformanceGuards = async (filters: IGuardReportFilters): Promise<TResult<any>> => {
    try {
        ...
        const groupData = await prisma.kardex.groupBy({
            ...
            where: {
                timestamp: { gte: start, lte: end },
                user: { role: { in: OPERATIONAL_ROLES } }
            },
            _count: { _all: true },
